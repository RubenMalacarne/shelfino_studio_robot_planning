...
namespace planning_pkg
{
...

  {
    ...
    // 1. Connetti start e goal ai nodi vicini validi
    ...
    // 2. connetti start ai nodi più vicini
    ...
    // 2. connetti goal ai nodi più vicini
    ...
    // 11. Esegui Dijkstra
    std::vector<int> path_indices = CommonFunction::dijkstra_shortest_path_temp(start_node, goal_node, temp_points, temp_adj);
  ...
  planning_pkg::PrmPathGenerator::indices_to_polyline(const std::vector<int> &idx_path) const
  {
    ...
    for (int id : idx_path)
    {
      if (id < 0 || static_cast<std::size_t>(id) >= random_points.size())
        continue;
