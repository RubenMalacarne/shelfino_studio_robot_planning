...
namespace
{
    ...
    std::vector<double> line_segment_intersection(double y, double x1, double y1, double x2, double y2)
    {
    ...
    std::vector<double> line_circle_intersection(double y, double center_x, double center_y, double radius)
    {
        std::vector<double> intersections;
...
namespace planning_pkg
{
        ...
        const geometry_msgs::msg::Polygon &arena)
    {
        ...
        for (size_t i = 0; i < start_connections; ++i)
        {
            ...
            if (point_is_valid((start_point.x + temp_points[neighbor].x) / 2.0,
        ...
        for (size_t i = 0; i < goal_connections; ++i)
        {
            ...
            if (point_is_valid((goal_point.x + temp_points[neighbor].x) / 2.0,
        ...
        if (direct_dist <= max_connection_distance &&
            point_is_valid((start_point.x + goal_point.x) / 2.0,
        ...
        for (const auto &segment : arena_segments)
        {
            ...
            for (const auto &obstacle : obstacles.obstacles)
            {
                if (obstacle.radius > 0.0)
                {
                    ...
                    auto intersections = line_circle_intersection(y, center_x, center_y, obstacle.radius);
                ...
                else
                {
                    ...
                    for (size_t i = 0; i < poly.points.size(); ++i)
                    {
                        ...
                        auto intersections = line_segment_intersection(y, x1, y1, x2, y2);
    ...
    const geometry_msgs::msg::Polygon &arena) const
{
    ...
    for (size_t i = 0; i < arena.points.size(); ++i)
    {
        ...
        auto line_intersections = line_segment_intersection(y, x1, y1, x2, y2);
