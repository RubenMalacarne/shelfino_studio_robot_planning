...
private:
    ...
    bool is_point_in_obstacle(double x, double y, const obstacles_msgs::msg::ObstacleArrayMsg &obstacles, double safety_margin = 0.1)
    {
        for (const auto &obstacle : obstacles.obstacles)
        {
            ...
            // Check distance to edges for safety margin
            for (size_t i = 0; i < obstacle.polygon.points.size(); ++i)
            {
                ...
                if (dist_to_edge < safety_margin) return true;
            }
        }
        return false;
    }
    ...
    {
        if (arena.points.empty()) return true; // No arena constraint
    ...
    double point_to_line_distance(double px, double py, double x1, double y1, double x2, double y2)
    {
        ...
        if (len_sq < 1e-6) return sqrt(A * A + B * B);
        ...
        const std::string &frame_id)
    {
        ...
        // Convert to KDPoint format
        std::vector<KDPoint> kd_waypoints;
        
        // Add robot position with its orientation
    ...
    std::vector<KDPoint> generate_safe_dubins_path(const std::vector<KDPoint> &waypoints)
    {
        ...
        // Generate Dubins curves between consecutive waypoints
        for (size_t i = 0; i < waypoints.size() - 1; ++i)
        {
            ...
            for (double radius : radii_to_try)
            {
                ...
                {
                    // Check if this segment is collision-free
                    ...
                    if (segment_valid)
                    {
                        // Add this segment to the path (avoid duplicating the start point)
                        if (safe_path.empty())
                        {
            ...
            if (!path_found)
            {
                ...
                // Fallback: add straight line segment with dense sampling
